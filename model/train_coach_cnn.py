...
class CNNBiLSTM(nn.Module):
    def __init__(self, in_feats, cnn_channels=256, lstm_hidden=256,lstm_layers=2, dropout=0.5, num_type=5, num_form=None):
        super().__init__()
        ...
        self.lstm = nn.LSTM(cnn_channels, lstm_hidden, num_layers=lstm_layers, batch_first=True, bidirectional=True, dropout=0.0 if lstm_layers == 1 else dropout)
...
def train(args):
    ...
    summary_csv_default = root / "dataset" / "windows_phase2_summary.csv"
    summary_csv = Path(args.summary_csv) if args.summary_csv else summary_csv_default
    if not summary_csv.exists():
        alt = Path(os.path.splitext(args.data_npz)[0] + "_summary.csv")
        if alt.exists():
            summary_csv = alt
        else:
            raise FileNotFoundError(f"Could not find summary CSV: {summary_csv}")
    meta = pd.read_csv(summary_csv, encoding="utf-8")
    if "window_idx" in meta.columns:
        meta = meta.sort_values("window_idx").reset_index(drop=True)
    if "video" not in meta.columns and "video_norm" in meta.columns:
        meta = meta.rename(columns={"video_norm": "video"})
    N = X.shape[0]
    if len(meta) != N:
        meta = meta.iloc[:N].copy()
    assert len(meta) == len(X), "summary rows must match number of windows"

    groups = meta["video"].astype(str).values
    ...
    if args.grouped_split:
        ...
        if args.grouped_strategy == "stratified":
            ...
            train_idx, val_idx, _ = make_grouped_split_all_classes(X, y_type, groups, seed=args.seed, val_ratio=args.val_ratio)
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if use_form and len(preds_form) > 0:
            ...
            print(f"Epoch {epoch}/{args.epochs}  loss={total_loss/len(train_loader):.4f}  type: acc={acc_type:.3f} f1={f1_type:.3f}  form: acc={acc_form:.3f} f1={f1_form:.3f}")
        else:
            print(f"Epoch {epoch}/{args.epochs}  loss={total_loss/len(train_loader):.4f}  type: acc={acc_type:.3f} f1={f1_type:.3f}")
        sched.step(f1_type)
...
def parse_args():
    ...
    ap.add_argument("--out_model", default="model/phase2_cnn_bilstm_v10.pt")
    ...
    ap.add_argument("--grouped_strategy", choices=["random","stratified"], default="stratified")
